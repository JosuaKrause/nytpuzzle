...
- **Image puzzles**: some Connections puzzles (e.g. 2026-05-06) use SVG images instead of text. `ConnectionsCard` has optional `image_url` + `image_alt_text` (image puzzle) or `content` (text puzzle). `GameCard` carries `imageUrl?`/`imageAlt?` alongside `content: string` (empty for image cards). Card face renders `<SvgUri>` when `imageUrl` is present, `<Text>` wrapped in a centering `<View>` otherwise (the wrapper is needed because importing `react-native-svg` causes Pressable's `justifyContent: 'center'` to not center `Text` children on the new architecture). Solved-row words use `imageAlt ?? content` from `state.cards`. `react-native-svg` is mocked as `{ SvgUri: () => null }` in `jest.setup.ts`.
- **Grid vertical centring**: the grid (`Animated.View`) is wrapped in a `gridWrapper` View (`flex: 1, justifyContent: 'center'`) rather than putting `justifyContent: 'center'` on `boardContainer`. This is required — `justifyContent: 'center'` on any direct ancestor of the grid breaks LayoutAnimation position resolution on the new architecture (Fabric).
